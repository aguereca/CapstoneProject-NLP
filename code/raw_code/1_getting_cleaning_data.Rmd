...
for (media in sourcesMedia) {
    if (!file.exists(filePath(paste(media,'lines.all', sep='.'), 'en_US'))) {
        ...
        print(paste(media, system.time(
            {tokenizer(filePath(paste(media,'txt', sep='.'), 'en_US'),
                       filePath(paste(media,'lines.all', sep='.'), 'en_US'))}
            ), sep=' - '))
    }
}
```
...
```{r}
readLines(filePath('news.lines.all', 'en_US'), 10)
```
...
Having files with cleanly defined sententences we'll split our data source into training and test sets. 
For testing we are going to use the last 100K lines of each `lines.all` file, we find no need to randomize the lines since our data sources are already randomized. 

We use this bash code to split our data and generate respective `*.lines.train` and `*.lines.test` files for each media type. On subsecuent sections we'll operate on the train data set unless otherwise indicated.

```{bash, eval=FALSE}
# bash code should be runned from localized data folder
all_media=("blogs" "news" "twitter") && 
for media in ${all_media[@]}; do test_size=100000 && ghead -n -$test_size en_US.$media.lines.all > en_US.$media.lines.train && 
tail -n $test_size en_US.$media.lines.all > en_US.$media.lines.test; done && 
unset all_media && unset test_size
```
...
Most of the files seem to have tens of million of ngrams, their frequency distribution is expected to be exponential, for validation lets plot histograms of their counts:
